...
### Changed
- Added `.github/governance/provider-surface-projection.catalog.json` plus `.github/schemas/provider-surface-projection.catalog.schema.json` as the canonical projection map for authored `definitions/` assets, generated exceptions, projected runtime surfaces, and renderer ownership; added `scripts/common/provider-surface-catalog.ps1` plus `scripts/runtime/render-provider-surfaces.ps1`; switched `scripts/runtime/bootstrap.ps1` and `scripts/validation/validate-instructions.ps1` to consume that catalog instead of keeping a second hardcoded provider-surface map; and extended `scripts/tests/runtime/runtime-scripts.tests.ps1`, `definitions/README.md`, `README.md`, and `scripts/README.md` to document and validate the dispatcher path.
- Refined the GitHub authority boundary so reusable instructions/templates now live under `definitions/shared/`, while GitHub-native repository/community assets remain authored directly in `.github/`. The rendered GitHub/Copilot provider surface is now limited to `definitions/providers/github/{root,agents,chatmodes,prompts,hooks}/` plus `definitions/shared/{instructions,templates}/`; `.github/PULL_REQUEST_TEMPLATE.md`, `.github/ISSUE_TEMPLATE/**`, `.github/dependabot.yml`, and `.github/dependency-review-config.yml` are no longer provider-authored projection inputs.
- Moved the reusable POML prompt library out of `definitions/providers/github/prompts/poml/` into `definitions/shared/prompts/poml/`, kept `.github/prompts/poml/` as the projected runtime/editor surface, and narrowed `definitions/providers/github/prompts/` to provider-specific `*.prompt.md` entrypoints only.
