...
The canonical projection map lives in `.github/governance/provider-surface-projection.catalog.json`. Use `scripts/runtime/render-provider-surfaces.ps1` when you want one dispatcher to select the correct renderer set from that catalog instead of invoking each renderer manually.
...
- `.github/governance/provider-surface-projection.catalog.json`: canonical map of authored `definitions/` assets, generated exceptions, projected targets, and renderer ownership
...
| `runtime/render-github-instruction-surfaces.ps1` | Regenerates the rendered GitHub instruction/runtime surface from `definitions/providers/github/{root,agents,chatmodes,prompts,hooks}/` plus `definitions/shared/{instructions,prompts/poml,templates}/` into `.github/`. Provider prompt entrypoints stay under `definitions/providers/github/prompts/*.prompt.md`, while the shared POML prompt library is projected into `.github/prompts/poml/`. GitHub-native issue templates, PR templates, and dependency automation config remain authored directly in `.github/`. | `pwsh -File scripts/runtime/render-github-instruction-surfaces.ps1 -RepoRoot .` |
| `runtime/render-provider-surfaces.ps1` | Reads `.github/governance/provider-surface-projection.catalog.json` and dispatches the correct renderer set for direct or bootstrap consumption. Use it when you want the canonical provider-surface selection logic in one place instead of manually invoking each renderer. | `pwsh -File scripts/runtime/render-provider-surfaces.ps1 -RepoRoot . -ConsumerName bootstrap -EnableCodexRuntime -EnableClaudeRuntime` |
...
| `runtime/render-vscode-workspace-surfaces.ps1` | Regenerates the rendered `.vscode/` workspace-authored surface (`README.md`, `base.code-workspace`, `settings.tamplate.jsonc`, `snippets/`) from `definitions/providers/vscode/workspace/`. Use it after workspace-definition edits or in parity checks. | `pwsh -File scripts/runtime/render-vscode-workspace-surfaces.ps1 -RepoRoot .` |
...
| `runtime/invoke-super-agent-housekeeping.ps1` | Runs safe periodic housekeeping for one workspace. It refreshes the local context index, exports a concise planning handoff, then cleans persisted Codex and VS Code runtime state, while never attempting to clear the live active context window or UI token meter. Hook-driven dispatch is throttled per workspace every 2 hours by default. | `pwsh -File scripts/runtime/invoke-super-agent-housekeeping.ps1 -WorkspacePath . -Apply` |
...
| `runtime/export-planning-summary.ps1` | Exports a context handoff summary from active planning artifacts (`planning/active/` and `planning/specs/active/`). Generates a structured markdown document with active plans, active specs, recent git commits, and suggested local indexed references when available, so sessions can resume from a known stable state without replaying large history. Saves to `.temp/context-handoff-<timestamp>.md` or prints to console with `-PrintOnly`. | `pwsh -File scripts/runtime/export-planning-summary.ps1 -PrintOnly` |
...
- EOF normalization:
  ...
  - when the local context index already exists, the same hook summary also injects a short `Local context refs:` segment pointing to the next best repository files to reopen
...
- periodic Super Agent housekeeping refreshes the local context index first, then exports a concise planning summary, then cleans only persisted Codex and VS Code runtime state
